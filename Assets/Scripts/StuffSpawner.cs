...
public class StuffSpawner : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        float placeObstacle = Random.Range(0f, 1f); //50% chances
        int obstacleIndex = -1;

        if (placeObstacle <= 0.9)
        {
            ...
            for (int i = 0; i < (int)(Time.timeSinceLevelLoad / 60) + 1; i++)
            {
                CreateObject(StuffSpawnPoints[i].position, Obstacles[Random.Range(0, Obstacles.Length)]);
            }
        }
    ...
    void CreateObject(Vector3 position, GameObject prefab)
    {
        if (RandomX) //true on the wide path, false on the rotated ones
            position += new Vector3(Random.Range(minX, maxX), -0.1f, 0);
